...
from dyndesign import importclass, mergeclasses, DynInheritance
from dyndesign.exceptions import ErrorClassNotFoundInModules
from .test_nested1.test_nested2.additional_test_dyn_inheritance import AdditionalDynIneritanceTests
...
def test_add_parents_locked_instances():
    ...
    `B_Locked` is enabled to dynamically modify its superclass set while keeping locked the class in the existing class
    instances. In greater details, `B_Locked` is instantiated in `original_instance` first, then class `A` is appended
    to its superclass set, and finally `B_Locked` is instantiated again in `inheriting_instance`. This test shows that
    `original_instance` is still an instance of the initial class `B_Locked`, before that parent `A` is appended.
    """
    original_instance = B_Locked()

    B_Locked.dynparents_add(A)
    inheriting_instance = B_Locked()
    assert inheriting_instance.a1 == cr.CLASS_B__A1, "Error initializing attribute `a1`"
    assert inheriting_instance.a2 == cr.CLASS_A__A2, "Error initializing attribute `a2`"
    assert inheriting_instance.a3 == cr.CLASS_B__M1, "Error initializing attribute `a3`"
    assert inheriting_instance.m1() == cr.CLASS_B__M1, "Error overloading method `m1`"
    ...
    B_Locked.dynparents_restore()


def test_add_parents_rename_locked_instances():
    """Test similar to "test_add_parents_live_instances", but in this case the initial class `B_Locked` is kept
    separate from the patched class that is renamed to `NewB`. By this way, the initial class `B_Locked` is
    instantiated as the instance `original_instance` after the call to `dynparents_add`.
    """
    B_Locked.dynparents_add(A, rename_to="NewB")
...
def test_add_parents_keep_docstring():
    """This test verifies that the renamed class `NewB` retains the same docstring as the original class `B_Locked`.
    """
    B_Locked.dynparents_add(A, rename_to="NewB")
    assert NewB.__doc__ == "B_Locked docstring"
...
def test_restore_parents():
    ...
    and in all its class instances. The API `safesuper`, which ensures the safe retrieval of parent class `C` when it
    is dynamically added, is also tested in the constructor of class `D`.
    """
    D.dynparents_add(C)
...
def test_restore_parents_locked_instances():
    ...
    """
    D_Locked.dynparents_add(C)
    inheriting_instance = D_Locked()

    D_Locked.dynparents_restore()
    original_instance = D_Locked()
...
def test_add_parents_dynamic_import_locked_instances_fails():
    ...
    """
    E_Locked = importclass('tests.samples.sample_classes_imported.E_Locked')
    with pytest.raises(ErrorClassNotFoundInModules):
        E_Locked.dynparents_add('tests.samples.sample_classes_imported.A')
...
def test_dynamic_import_of_added_parents_locked_instances():
    ...
    test.
    """
    F_Locked.dynparents_add('tests.samples.sample_classes_imported.A')
    inheriting_instance = F_Locked()
    assert inheriting_instance.a1 == cr.CLASS_A__A1, "Error initializing attribute `a1`"
    assert inheriting_instance.m1() == cr.CLASS_A__M1, "Error overloading method `m1`"
    assert inheriting_instance.m2() == cr.CLASS_F__M2, "Error overloading method `m2`"

    F_Locked.dynparents_restore()


def test_add_parents_nested_dir_locked_instances():
    """Class `B_Locked`, which is loaded from the loader class `AdditionalDynIneritanceTests` of module
    `additional_test_dyn_inheritance`, undergoes an update to its superclass set with locked instances.
    """
    B_Locked = AdditionalDynIneritanceTests.get_B_Locked()
    inheriting_instance = B_Locked()
    assert inheriting_instance.a1 == cr.CLASS_B__A1, "Error initializing attribute `a1`"
    assert inheriting_instance.a2 == cr.CLASS_A__A2, "Error initializing attribute `a2`"
    assert inheriting_instance.a3 == cr.CLASS_B__M1, "Error initializing attribute `a3`"
...
def test_auto_add_parents_and_mocked_methods():
    ...
    passed as `mocked_methods` argument of `safesuper` API from method instance `G.m1`. The method's call can therefore
    be safely chained to the `safesuper` call, even when `G` does not inherit from `A`.
    """
...
def test_add_parents_dyn_decorators():
    ...
    N.dynparents_add(M)
    assert inheriting_instance.m1() == [cr.CLASS_N__ITEM_1, cr.CLASS_M__ITEM_1, cr.CLASS_N__ITEM_1], "Error calling method `m1`"

    N.dynparents_restore()


def test_cannot_remove_dyn_inherit_class():
    """"This test demonstrates that the special class `DynInheritance` cannot be dynamically removed from the superclass set."
    """
